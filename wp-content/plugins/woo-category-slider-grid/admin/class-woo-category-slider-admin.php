...
class Woo_Category_Slider_Admin {
	...
	public function add_plugin_action_links( $links, $file ) {
		...
		if ( $file === SP_WCS_BASENAME ) {
			...
			$ui_links = sprintf( '<a href="%s">%s</a>', admin_url( 'post-new.php?post_type=sp_wcslider' ), __( 'Create Slider', 'woo-category-slider-grid' ) );
	...
	public function woo_gallery_slider_admin_notice() {
		...
		if ( current_user_can( 'install_plugins' ) && 'sp_wcslider' === $the_current_post_type ) {
			...
			$button_text = esc_html__( 'Install', 'woo-category-slider-grid' );
